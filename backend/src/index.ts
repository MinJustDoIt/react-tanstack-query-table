...
export const seedData = () => {
  ...
  for (let i = 0; i < 1000; i++) {
    const randomDepartment = departments[Math.floor(Math.random() * departments.length)].name;
...
const paginate = (items: Employee[], page: number, perPage: number): Employee[] => {
...
app.get('/employees', (req: Request, res: Response) => {
  let { page = '1', limit = '10', sort = 'id', filterName = '', filterDepartment = '', filterEmployedBefore = '', filterEmployedAfter = '' } = req.query;
  ...
  if (filterDepartment) {
    filteredEmployees = filteredEmployees.filter(emp =>
      emp.department.toLowerCase().includes((filterDepartment as string).toLowerCase())
    );
  }
...
app.post('/employees', (req: Request, res: Response) => {
  ...
  if (!name || !age || !department || !position || !employedOn) {
    return res.status(400).json({ message: 'All fields are required: name, age, department, position, employedOn' });
  }
